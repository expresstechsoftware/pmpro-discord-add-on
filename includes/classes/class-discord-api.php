...
class PMPro_Discord_API {
	 ...
	 * @return OBJECT REST API response
	 */
	public function ets_pmpro_discord_load_discord_roles() {
		...
		if ( $guild_id && $discord_bot_token ) {
			...
			if ( is_array( $response_arr ) && ! empty( $response_arr ) ) {
				if ( array_key_exists( 'code', $response_arr ) || array_key_exists( 'error', $response_arr ) ) {
					...
					$response_arr['previous_mapping'] = get_option( 'ets_pmpor_discord_role_mapping' );
	...
	*/
	...
	/**
	 * Manage user roles on subscription  payment complete
	 *
	 * @param ARRAY $morder
	 */
	public function ets_pmpro_subscription_payment_completed( $morder ) {
